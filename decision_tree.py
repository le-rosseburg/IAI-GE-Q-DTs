...
class DecisionTree:
    ...
    ----------
    program : str
        the phenotype of the decision tree
    current_reward : float
        the current reward of the decision tree
    leaves : dictionary<leaf>
        a dictionary containing all existing leaves
    leaf_count : int
        total number of existing leaves
    last_leaf : leaf
        the last leaf visited
    exec_ : code object
        executable code object generated from phenotype

    Methods
    -------
    get_acion(input):
        returns optimal action of the current leaf

    set_reward(reward):
        sets current_reward of the decision tree to reward

    new_episode():
        resets variable last_leaf to None
    """
